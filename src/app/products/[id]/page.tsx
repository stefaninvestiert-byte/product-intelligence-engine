...
export default async function ProductDetailPage({ params }: Params) {
  ...
  return (
    <div className="p-6 space-y-6 max-w-7xl">
      ...
      {/* New 4-Dimension Winner Score Breakdown */}
      {s && (
        <div className="card p-5">
          <h3 className="text-white font-bold mb-4">🏆 Winner Score Breakdown <span className="text-slate-400 font-normal text-sm ml-2">— neue Formel: 35% PS + 25% Competition + 25% Demand + 15% Profit</span></h3>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="card p-5">
            ...
            <div className="space-y-2.5">
              ...
              <ScoreBar
                score={100 - s.beforeAfterRisk}
                label="Vorher-Nachher Sicherheit"
                size="sm"
              />
              <ScoreBar
                score={100 - s.complianceRisk}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="card p-5">
            ...
            <div className="space-y-3">
              ...
              {r.emotionalTriggers && r.emotionalTriggers.length > 0 && (
                <div>
                  <div className="text-slate-500 text-xs font-semibold uppercase mb-2">Emotionale Trigger</div>
                  <div className="flex flex-wrap gap-2">
                    {r.emotionalTriggers.map((t, i) => (
                      <span key={i} className="badge bg-red-900/30 text-red-300 border border-red-500/30 text-xs px-3 py-1">
                        ⚡ {t}
                      </span>
                    ))}
                  </div>
          ...
          <div className="card p-5">
            ...
            <div className="space-y-2">
              {r.hooks.map((hook, i) => (
                <div
                  key={i}
                  className="bg-slate-900/60 border border-slate-700 rounded-lg p-3 text-slate-300 text-sm cursor-pointer hover:border-blue-500/50 transition-colors"
          ...
          <div className="card p-5">
            ...
            <div className="space-y-2">
              {r.headlines.map((h, i) => (
                <div
                  key={i}
                  className="bg-slate-900/60 border border-slate-700 rounded-lg p-3 text-slate-300 text-sm cursor-pointer hover:border-purple-500/50 transition-colors"
          ...
          <div className="card p-5">
            ...
            <div
              ...
              onClick={() => r.primaryText && navigator.clipboard.writeText(r.primaryText)}
              title="Klicken zum Kopieren"
            >
              {r.primaryText}
            </div>
        ...
        <button
          onClick={() =>
            fetch("/api/export", {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({ format: "EXCEL", productIds: [product.id] }),
            })
              .then((r) => r.blob())
              .then((blob) => {
                ...
                a.href = url;
                a.download = `${product.name}-${Date.now()}.xlsx`;
                a.click();
              })
          }
